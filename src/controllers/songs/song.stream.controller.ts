...
export const streamSong = (req: TStreamSongRequest, reply: FastifyReply) => {
    ...
    // ✅ Ключевое исправление: если нет Range — отправляем полный файл
    if (!range) {
        reply.headers({
            "Content-Length": fileSize.toString(),
            "Content-Type": "audio/mpeg",
            "Accept-Ranges": "bytes", // ← КРИТИЧЕСКИ ВАЖНО: говорит плееру, что сервер поддерживает диапазоны
        });
    ...
    const end = parts[1] ? parseInt(parts[1], 10) : Math.min(start + 1_000_000, fileSize - 1);
